...
console.log('\n# Nuvio settings template resolution');
{
  ...
  ok('TV template includes debrid stream name template', typeof tv?.settingsJson?.features?.debrid_settings?.debrid_stream_name_template?.value === 'string');
  ok('Mobile template includes debrid stream description template', typeof mobile?.settingsJson?.features?.debrid_settings?.debrid_stream_description_template?.value === 'string');
...
console.log('\n# Nuvio TV settings merge');
{
  ...
  ok('TV merge adds debrid stream name template', merged.features.debrid_settings.debrid_stream_name_template.value.includes('{service.cached::istrue'));
  ok('TV merge adds debrid stream description template', merged.features.debrid_settings.debrid_stream_description_template.value.includes('{stream.edition::exists'));
...
console.log('\n# Nuvio mobile TMDB merge');
{
  ...
  ok('Mobile merge adds debrid stream name template', merged.features.debrid_settings.debrid_stream_name_template.value.includes('{service.shortName}'));
  ok('Mobile merge adds debrid stream description template', merged.features.debrid_settings.debrid_stream_description_template.value.includes('{stream.languages::exists'));
